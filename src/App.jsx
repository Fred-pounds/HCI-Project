...
import { Menu, Search, Trash2, AlertTriangle } from "lucide-react";

export default function ChatApp() {
  const [messages, setMessages] = useState([
    { id: 1, text: "Rephrase 'This is an AI chatbot generated for better communication and simpler work flows'", sender: "user" },
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className="w-1/4 bg-[#0A0C19] text-white p-4 flex flex-col">
        ...
        <nav className="flex-1">
          <ul>
            <li className="p-2 hover:bg-gray-700 rounded bg-[#282934] border-black-300">New Chat</li><br></br>
        ...
        <div className="p-4 border-t border-gray-700">Welcome back, <strong>Fred</strong></div>
      </aside>
      ...
      <main className="flex-1 flex flex-col p-4">
        ...
        <div className="flex-1 p-4 space-y-2 overflow-y-auto">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === "user" ? "justify-end" : "justify-start"}` }>
              <div className={`p-3 rounded-lg ${msg.sender === "user" ? "bg-blue-500 text-white" : "bg-gray-200 text-black"}`}>{msg.text}</div>
            </div>
          ))}
        </div>
        ...
        <div className="p-2 border-t border-black-300 flex items-center gap-2">
          <input type="text" placeholder="Type a new message here" className="flex-1 p-2 border rounded text-black opacity-70" />
